...
new List:PropertyObjects[MAX_PROPERTIES];
...
hook OnPropertiesLoaded() {
	...
	{
		new rows, total_objects[MAX_PROPERTIES];
    ...
    mysql_tquery_inline(handle_db, "SELECT * FROM property_objects WHERE `create` = 1;", using inline OnQueryLoaded);
}
...
CountPropertyObjects(id_property) {
    mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "SELECT COUNT(*) FROM property_objects WHERE id_property = %d;", id_property);
...
CountPropertyUncreatedObjects(id_property) {
    mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "SELECT COUNT(*) FROM property_objects WHERE id_property = %d AND `create` = 0;", id_property);
    new count, Cache:result = mysql_query(handle_db, QUERY_BUFFER);
    cache_get_value_int(0, 0, count);
    cache_delete(result);
    return count;
...
RegisterPropertyObject(property_index, id_property, modelid, create = 0, Float:x = 0.0, Float:y = 0.0, Float:z = 0.0, Float:rx = 0.0, Float:ry = 0.0, Float:rz = 0.0, texture0[] = "", texture1[] = "", texture2[] = "", texture3[] = "", bool:editWhenCreate = false, forplayerid = INVALID_PLAYER_ID) {
    ...
    mysql_tquery_inline(handle_db, QUERY_BUFFER, using inline OnQueryLoaded, "ssss", texture0, texture1, texture2, texture3);
}
...
DeletePropertyObjects(property_index, bool:db_delete = true) {
    ...
    if(db_delete) {
        mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "DELETE FROM property_objects WHERE id_property = %d;", PROPERTY_INFO[property_index][property_ID]);
...
DeletePropertyObject(property_index, id_object, objectid, bool:uncreate = false) {
    ...
    if(uncreate) {
        mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "UPDATE property_objects SET `create` = 0 WHERE id = %d;", id_object);
        mysql_tquery(handle_db, QUERY_BUFFER);
    }
    else {
        mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "DELETE FROM property_objects WHERE id = %d;", id_object);
        mysql_tquery(handle_db, QUERY_BUFFER);
    }
}
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) {
    switch(dialogid) {
        ...
        case DIALOG_POBJECT_EDIT: {
            if(response) {
                switch(listitem) {
                    case 0: EditDynamicObject(playerid, pPObjectCurrentObjectID[playerid]);
                    case 1: { //texturizar
                    ...
                    case 2: { //clonar
                        ...
                        if(CountPropertyObjects(PROPERTY_INFO[property_index][property_ID]) >= limit) SendNotification(playerid, "No puedes añadir más muebles a esta propiedad.");
                            ...
                            if(GivePlayerCash(playerid, -GetPObjectModelPrice(pPObjectCurrentModelID[playerid]), true, true)) {
                                ...
                                mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "SELECT * FROM property_objects WHERE id = %d;", pPObjectCurrentID[playerid]);
                                mysql_tquery_inline(handle_db, QUERY_BUFFER, using inline OnQueryLoaded);
                            }
        ...
        if(playertextid == pUncreatedPObjectsTd[playerid][9]) { //<<
            ...
            for(new i = 0; i < UNCREATED_POBJECTS_PER_PAGE; i ++) {
                if(playertextid == pUncreatedPObjectsTd[playerid][mdlTdStartIndex]) {
                    ...
                    mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "SELECT * FROM property_objects WHERE id = %d;", object_id);
                    mysql_tquery_inline(handle_db, QUERY_BUFFER, using inline OnQueryLoaded);
                    return Y_HOOKS_BREAK_RETURN_1;
                }
                mdlTdStartIndex ++;
...
UpdatePUObjectsTextDraws(playerid) {
    ...
    mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "SELECT id, modelid FROM property_objects WHERE id_property = %d AND `create` = 0 LIMIT "#UNCREATED_POBJECTS_PER_PAGE" OFFSET %d;", id_property, pPUObjectsMenuCurrentPage[playerid] * UNCREATED_POBJECTS_PER_PAGE);
    mysql_tquery_inline(handle_db, QUERY_BUFFER, using inline OnQueryLoaded);
}
...
UpdatePObjectTexture(id_object, objectid, materialindex, modelid, txdname[], texturename[], color) {
    ...
    mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "UPDATE property_objects SET texture%d = '%e' WHERE id = %d;", materialindex, string, id_object);
    mysql_tquery(handle_db, QUERY_BUFFER);
}
...
DeletePObjectTexture(property_index, id_object, objectid, materialindex, bool:menuDialogWhenCreate = false, forplayerid = INVALID_PLAYER_ID) {
    ...
    mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "SELECT * FROM property_objects WHERE id = %d;", id_object);
    mysql_tquery_inline(handle_db, QUERY_BUFFER, using inline OnQueryLoaded);
}
...
hook OnDefaultPFurChanged(index) {
    ...
    mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "UPDATE property_objects SET z = z + (%f) WHERE x != 0.0 AND y != 0.0 and z != 0.0 AND id_property = %d;", (PROPERTY_INFO[index][property_DIS_DEFAULT_INTERIOR] ? PROPERTY_EMPTY_INTERIOR_Z_OFFSET : -PROPERTY_EMPTY_INTERIOR_Z_OFFSET), PROPERTY_INFO[index][property_ID]);
    mysql_tquery(handle_db, QUERY_BUFFER);
}