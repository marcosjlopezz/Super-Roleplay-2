...
hook OnTerritoriesLoaded() {
    ...
    mysql_tquery_inline(handle_db, "SELECT * FROM graffiti;", using inline OnQueryLoaded);
}
...
CreateGraffitiFromInfo(id, Float:x, Float:y, Float:z, Float:rx, Float:ry, Float:rz, interior, world, modelid, const text[], const font[], font_size, font_bold, font_color, id_territory, id_crew, id_player, territoryIndex, crewIndex) {
...
hook OnCrewDeleted(crewId) {
    ...
    mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "DELETE FROM graffiti WHERE id_crew = %d;", crewId);
...
ClearTerritoryGraffitis(territoryId, territoryIndex) {
    ...
    mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "DELETE FROM graffiti WHERE id_territory = %d;", territoryId);
    mysql_tquery(handle_db, QUERY_BUFFER);
}
            ...
            if(response) {
                switch(listitem) {
                    ...
                    case 2: {
                        ...
                        if(index != -1) {
                            mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "DELETE FROM graffiti WHERE id = %d;", info[3]);
                            mysql_tquery(handle_db, QUERY_BUFFER);
                ...
                switch(listitem) {
                    ...
                    case 3: { //negrita
                        ...
                        if(index != -1) {
                            ...
                            mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "UPDATE graffiti SET font_bold = %d WHERE id = %d;", ginfo[graffiti_FONT_BOLD], ginfo[graffiti_ID]);
                            mysql_tquery(handle_db, QUERY_BUFFER);
        ...
        case DIALOG_GRAFFITI_ETEXT_TEXT: {
            if(response) {
                ...
                if(index != -1) {
                    ...
                    mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "UPDATE graffiti SET text = '%e' WHERE id = %d;", ginfo[graffiti_TEXT], ginfo[graffiti_ID]);
                    mysql_tquery(handle_db, QUERY_BUFFER);
                }
            }
            ShowEditGraffitiTextDialog(playerid);
        ...
        case DIALOG_GRAFFITI_ETEXT_FONT: {
            if(response) {
                ...
                if(index != -1) {
                    ...
                    mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "UPDATE graffiti SET font = '%e' WHERE id = %d;", ginfo[graffiti_FONT], ginfo[graffiti_ID]);
                    mysql_tquery(handle_db, QUERY_BUFFER);
                }
            }
            ShowEditGraffitiTextDialog(playerid);
        ...
        case DIALOG_GRAFFITI_ETEXT_SIZE: {
            if(response) {
                ...
                if(index != -1) {
                    ...
                    mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "UPDATE graffiti SET font_size = %d WHERE id = %d;", ginfo[graffiti_FONT_SIZE], ginfo[graffiti_ID]);
                    mysql_tquery(handle_db, QUERY_BUFFER);
                }
            }
            ShowEditGraffitiTextDialog(playerid);
        ...
        case DIALOG_GRAFFITI_ETEXT_COLOR: {
            if(response) {
                ...
                if(index != -1) {
                    ...
                    mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "UPDATE graffiti SET font_color = %d WHERE id = %d;", ginfo[graffiti_FONT_COLOR], ginfo[graffiti_ID]);
                    mysql_tquery(handle_db, QUERY_BUFFER);
                }
            }
            ShowEditGraffitiTextDialog(playerid);
    ...
    if(info[0] == OBJECT_TYPE_GRAFFITI && objectid == PLAYER_TEMP[playerid][pt_SELECTED_GRAFFITI]) {
        switch(response) {
            ...
            case EDIT_RESPONSE_FINAL: {
                ...
                mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "UPDATE graffiti SET x = %f, y = %f, z = %f, rx = %f, ry = %f, rz = %f WHERE id = %d;", x, y, z, rx, ry, rz, info[3]);
                mysql_tquery(handle_db, QUERY_BUFFER);
