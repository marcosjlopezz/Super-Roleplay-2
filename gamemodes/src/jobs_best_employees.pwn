		...
		if(cache_get_row_count(rows))
		{
			for(new i = 0; i != rows; i ++)
			{
                ...
                if(last_prize > 0) {
                    ...
                    mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "UPDATE pworks_points SET last_prize = 0 WHERE id_player = %d AND id_work = %d;", PI[playerid][pi_ID], work);
                    mysql_tquery(handle_db, QUERY_BUFFER);
                }
            }
        }
    }
    mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "SELECT * FROM pworks_points WHERE id_player = %d;", PI[playerid][pi_ID]);
...
AddPlayerJobPoints(playerid, work, points = 1, bool:message = true) {
    ...
    mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "INSERT INTO pworks_points (points, id_player, id_work) VALUES(%d, %d, %d) ON DUPLICATE KEY UPDATE points = %d;",
    ...
    if(resetSecondsLeft <= 0) {
        ...
        for(new i = 1; i < sizeof(work_info); i ++) {
            inline OnQueryLoaded()
            {
                    ...
                    if(rows)
                    {
                        ...
                        for(new j = 0; j != rows; j ++)
				        {
                            ...
                            if(connected && PI[playerid][pi_ID] == pid) {
                                ...
                                mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "UPDATE pworks_points SET last_prize = last_prize + %d WHERE id_player = %d AND id_work = %d;", currentPrize, pid, i);
                                mysql_tquery(handle_db, QUERY_BUFFER);

                                mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "UPDATE player SET cash = cash + %d WHERE id = %d;", currentPrize, pid);
                                mysql_tquery(handle_db, QUERY_BUFFER);

                                if(j == 0 && work_info[i][work_info_TOP1_COINS_PRIZE] > 0) {
                                    mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "UPDATE player SET coins = coins + %d WHERE id = %d;", work_info[i][work_info_TOP1_COINS_PRIZE], pid);
                                    mysql_tquery(handle_db, QUERY_BUFFER);
                                }
                            }
                ...
                //resetear
                mysql_format(handle_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "UPDATE pworks_points SET points = 0 WHERE id_work = %d;", i);
                mysql_tquery(handle_db, QUERY_BUFFER);
